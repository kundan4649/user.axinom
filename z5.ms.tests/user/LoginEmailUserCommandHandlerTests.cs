...
namespace user
{
    /// <summary>Tests for login user with email command handler</summary>
    public class LoginEmailUserCommandHandlerTests
    {
        ...
        /// <summary>Ensure that handler return success with valid login email user command</summary>
        [Fact]
        public async Task LoginEmailUserCommand_Success()
        {
            // Arrange
            var command = new LoginEmailUserCommand { Country = "EE", Refresh = true };
            ...
            var handler = new LoginEmailUserCommandHandler(_authenticationService.Object, _tokenService.Object);
        ...
        /// <summary>Ensure that handler return error when reading user by email fails from user repository</summary>
        [Fact]
        public async Task LoginEmailUserCommand_Fail()
        {
            // Arrange
            var command = new LoginEmailUserCommand { Country = "EE", Refresh = true };
            var user = new UserEntity { Id = Guid.NewGuid() };
