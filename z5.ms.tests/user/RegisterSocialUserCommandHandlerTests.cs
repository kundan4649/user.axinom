...
namespace user
{
    ...
    public class RegisterSocialUserCommandHandlerTests
    {
        ...
        /// <summary>Ensure that handler return success with valid Register Social user command</summary>
        [Fact]
        public async Task RegisterSocialUserCommand_Success()
        {
            // Arrange
            var command = new RegisterSocialUserCommand { RegistrationCountry = "EE", Refresh = true };
            ...
            _tokenService.Setup(r => r.GetJwtToken(user.Id, command.RegistrationCountry, command.Refresh,0))
                .ReturnsAsync(Result<OAuthToken>.FromValue(new OAuthToken { AccessToken = token }));

        //    var handler = new RegisterSocialUserCommandHandler(_authenticationService.Object, _tokenService.Object);
        ...
        [Fact]
        public async Task RegisterSocialUserCommand_Fail()
        {
            // Arrange
            var command = new RegisterSocialUserCommand { RegistrationCountry = "EE", Refresh = true };
            var user = new UserEntity { Id = Guid.NewGuid() };
