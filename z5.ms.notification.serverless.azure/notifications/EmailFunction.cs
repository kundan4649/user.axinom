...
namespace z5.ms.user.serverless.azure.notifications
{
    ...
    public static class EmailFunction
    {
        ...
        {
            if (!MailboxAddress.TryParse(email, out var emailTo))
            {
                log.LogError($"SendEmail to {email} FAILED with message: Invalid email");
                return;
            }
            ...
            try
            {
                ...
                log.LogDebug($"It took {(DateTime.UtcNow - start).TotalMilliseconds} milliseconds to send email to {email}");
            }
            catch (Exception ex)
            {
